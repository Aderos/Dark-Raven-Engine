...
void main(void)
{
	...
	vNormal    = normalize((mWorldMatrix * vec4(normal, 0.0)).xyz);
	vWorld     = normalize((mWorldMatrix * vec4(position, 0.0)).xyz);
	vTangent   = normalize((mWorldMatrix * vec4(CalcTangent(normal), 0.0)).xyz);
	vBitangent = normalize((mWorldMatrix * vec4(CalcBitangent(normal, vTangent), 0.0)).xyz);
