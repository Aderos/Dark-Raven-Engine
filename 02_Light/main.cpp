...
#define NUM_SHADER 3
enum { LAMBERT = 0, PHONG, COOK_TORRANCE };
...
Math::Vector3f vLightDir;
Math::Vector4f vAmbObj;
Math::Vector3f vSpeObj;
Math::Vector3f vCamPos;
Math::Vector3f vCamRot;
...
void Initialize()
{
	...
	m_Obj[0]->Initialize("../Assets/Monkey.obj", "../Assets/Monkey.bmp");
	...
	vLightDir = Vector3f(-7.0f, 5.0f, 7.0f);
	vAmbObj  = Vector4f(0.0f);
	vSpeObj = Vector3f(1.0f);
}
...
void Display(void)
{
	...
	{
		case LAMBERT: m_Shader->UseShader(GLT_LAMBERT_SHADER, mMVP, m_Obj[0]->GetWorldMatrix(), vLightDir, vAmbObj, m_Obj[0]->GetColorTexture()); break;
		case PHONG: m_Shader->UseShader(GLT_PHONG_SHADER, mMVP, m_Obj[0]->GetWorldMatrix(), vLightDir, vAmbObj, vSpeObj, m_Obj[0]->GetColorTexture()); break;
		case COOK_TORRANCE: m_Shader->UseShader(GLT_COOK_TORRANCE_SHADER, mMVP, m_Pipeline->GetCameraMatrix() * m_Obj[0]->GetWorldMatrix(),
			m_Obj[0]->GetWorldMatrix(), m_Pipeline->GetCameraMatrix(), 
			vLightDir, m_Pipeline->GetCameraPosition(), vAmbObj, vSpeObj, m_Obj[0]->GetColorTexture()); break;
	}
...
int main(int argc, char** argv)
{
	...
	bar = TwNewBar("MENU");
	TwDefine(" MENU size='220 570' color='0 0 0' ");
	...
	TwAddVarRW(bar, "", TW_TYPE_COLOR3F, &vAmbObj, "label='Ambient' group='Material'");
	TwAddVarRW(bar, "", TW_TYPE_COLOR3F, &vSpeObj, "label='Specular' group='Material'");

	TwAddVarRW(bar, "", TW_TYPE_DIR3F, &vLightDir, "label='Direction' opened=true group='Light'");
